...
class VariationalAutoencoder(object):
    ...
    def __init__(self,
        ...
        lg.debug("*=========================================================")
        lg.debug("* Constructing a VariationalAutoencoder...")
        lg.debug("*=========================================================")
        ...
        lg.debug("*---------------------------------------------------------")
        lg.debug("*")
        lg.debug("*--> Number of input features   (dim_x)   : % 6d" % (self.dim_x))
        lg.debug("*--> Number of latent variables (dim_z)   : % 6d" % (self.dim_z))
        lg.debug("*")
        lg.debug("*--> L2 loss                              : %f" % (self.l2_loss))
        lg.debug("*")
        for var, dist in self.distributions.items():
            lg.debug("*--> %s distribution : %s" % (var, dist))
        lg.debug("*")
        ...
        with self.G.as_default():
            ...
            ## The encoder q(z|x)???
            self.encoder = FullyConnected(dim_output = 2*self.dim_z, hidden_layers = hidden_layers_qz, nonlinearity = nonlin_qz)

            ## The decoder p(x|z)???
            self.decoder = FullyConnected(dim_output =   self.dim_x, hidden_layers = hidden_layers_px, nonlinearity = nonlin_px)
        ...
        lg.debug("*=============================================================")
        lg.debug("* Finished constructing the VariationalAutoencoder.")
        lg.debug("*=============================================================")
    ...
    def _objective(self):
        lg.debug("*")
        lg.debug("*---------------------------------------------------------")
        lg.debug("* VariationalAutoencoder: Calling _objective()")
        lg.debug("*---------------------------------------------------------")
        lg.debug("*")
        ############
        ''' Cost '''
        ...
        lg.debug("*")
        lg.debug("*---------------------------------------------------------")
        lg.debug("* VariationalAutoencoder: Finished _objective()")
        lg.debug("*---------------------------------------------------------")
        lg.debug("*")
    ...
    def train(self,
        ...
        lg.debug("*")
        lg.debug("*---------------------------------------------------------")
        lg.debug("* VariationalAutoencoder: starting train()")
        lg.debug("*---------------------------------------------------------")
        lg.debug("*")
        ...
        if save_path is None:
            ...
            checkpoints_dir_full_path = os.path.abspath(os.path.join(log_dir, checkpoints_dir))
            if not os.path.isdir(checkpoints_dir_full_path):
                os.mkdir(checkpoints_dir_full_path)
            self.save_path = os.path.join(checkpoints_dir_full_path, 'model_VAE_{}-{}_{}.cpkt'.format(learning_rate,self.batch_size,time.time()))
        else:
        ...
        if True:
            ...
            for epoch in range(epochs):
                ...
                for x_batch in utils.feed_numpy( self.batch_size, x ):
                    ...
                    training_result = sess.run( [self.train_op, self.cost], feed_dict = { self.x: x_batch } )
                ...
                if epoch % print_every == 0:
                    ...
                    eval_log_lik, x_recon_eval = sess.run( [self.eval_log_lik, self.x_recon_eval], feed_dict = { self.x: x_valid } )
                    ...
                    utils.print_metrics(epoch+1, ['Training', 'cost', training_cost], ['Validation', 'log-likelihood', eval_log_lik] )
                    ...
                    if draw_img > 0 and epoch % draw_img == 0:
                        ...
                        for i,row in enumerate(axes):
                            ...
                            im = row[1].imshow(x_recon_sample[i].reshape(28, 28), vmin=0, vmax=1, cmap=sns.light_palette((1.0, 0.4980, 0.0549), input="rgb", as_cmap=True))
        ...
        lg.debug("*")
        lg.debug("*---------------------------------------------------------")
        lg.debug("* VariationalAutoencoder: train() finished!")
        lg.debug("*---------------------------------------------------------")
        lg.debug("*")
    ...
    def encode( self, x, sample = False ):
        if sample:
            ...
            return self.session.run( [self.z_sample], feed_dict = { self.x: x } )
        else:
    ...
    def vae( self, x ):
        return self.decode( self.encode(x)[0] )[0]
